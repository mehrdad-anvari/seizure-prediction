...
@MODELS.register("eegnet", help="EEGNet (Lawhern et al. 2018) baseline model.")
def build_eegnet(cfg: ModelConfig):
    kw = dict(getattr(cfg, "kwargs", {}) or {})
    num_electrodes = cfg.in_channels or kw.get("num_electrodes", 60)
    chunk_size = kw.get("chunk_size", kw.get("seq_len", 151))
    return EEGNet(
        ...
        kernel_1=int(kw.get("kernel_1", 64)),
        kernel_2=int(kw.get("kernel_2", 16)),
        dropout=float(kw.get("dropout", 0.25)),
    )
