...
try:
    ...
    rearrange = None  # type: ignore
    _EINOPS_IMPORT_ERROR = e
...
@MODELS.register("labram", help="LaBraM (transformer-style model; requires einops).")
def build_labram(cfg: ModelConfig) -> nn.Module:
    if rearrange is None:  # pragma: no cover
        raise RuntimeError(
            "LaBraM model requires the optional dependency 'einops'. "
