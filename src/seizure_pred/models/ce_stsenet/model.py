...
def _load_scaling_filter_mat(params_path=None):
    """Load the packaged CE-stSENet scaling_filter.mat (or a user-provided path)."""
    if _scipy_io is None:
        raise ImportError(
            "CE-stSENet requires SciPy to load wavelet filters. "
            "Install with: pip install seizure-pred[signal]"
        )
    if params_path is None or str(params_path).strip() == "":
        mat = _resources.files("seizure_pred.models.ce_stsenet.assets").joinpath("scaling_filter.mat")
        with _resources.as_file(mat) as p:
            return _scipy_io.loadmat(str(p))
    return _scipy_io.loadmat(str(params_path))
...
class MultiLevel_Spectral(nn.Module): 
    def __init__(self, inc, params_path=None):
        super(MultiLevel_Spectral, self).__init__()
        f = _load_scaling_filter_mat(params_path)
        self.filter_length = int(f["Lo_D"].shape[1])
        ...
        # Initialize with fixed (non-trainable) Daubechies wavelet filters from the .mat file.
        Lo_D = np.flip(f["Lo_D"], axis=1).astype("float32")
        Hi_D = np.flip(f["Hi_D"], axis=1).astype("float32")
        w = torch.from_numpy(np.concatenate((Lo_D, Hi_D), axis=0))
        w = w.unsqueeze(1).unsqueeze(1).repeat(inc, 1, 1, 1)
        with torch.no_grad():
            self.conv.weight.copy_(w)
        self.conv.weight.requires_grad_(False)

    def self_padding(self, x):
        # Circular padding on the time dimension (last dim of Conv2d kernel).
        p = self.filter_length // 2 - 1
        if p <= 0:
            return x
        return torch.cat((x[..., -p:], x, x[..., :p]), dim=3)
...
This file contains the original CE-stSENet implementation.

Registry registration is handled in :mod:`seizure_pred.models.ce_stsenet.ce_stsenet`
to avoid duplicate model registration when both modules are imported.
"""
